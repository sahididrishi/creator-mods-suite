...
| `assets/creator_colossus/textures/entity/ashen_colossus_enraged.png` | 128x128 RGBA PNG | **PROCEDURAL PLACEHOLDER** | The phase 3 variant — `client/ColossusModel.java` swaps to it when `getPhase() >= 3`. **Every painted pixel differs from the calm sheet**, not just the crack pixels: a scorched red-brown crust (`STONE_RAGE`), veins two pixels wide instead of one and a third crack column down the wide faces. It was briefly a 2-value hue shift on 902 crack pixels, which is invisible past about four blocks and made the plan's "cracks glow red-hot" beat a non-event on camera. Real art: cracks going white-hot and a crust that looks like it is failing. |
...
* **No sound or particle keyframes are used**, and none are needed. This is a deliberate, accepted deviation from plan section 5/section 6, which sketch `setSoundKeyframeHandler(new AutoPlayingSoundKeyframeHandler<>())` and a `slam_dust` particle keyframe. Every sound and particle in this
