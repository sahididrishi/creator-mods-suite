...
* **The chime is mono 44.1 kHz Ogg Vorbis.** It does not matter for the current call site — the
  sound is only ever played through `SimpleSoundInstance.forUI(...)` on the client that owns the
  HUD — but mono means it keeps working if the chime is ever moved to a world sound, where stereo
  would be played non-positionally. ffmpeg's native `vorbis` encoder is stereo-only, so it is
  encoded via `ffmpeg -ac 1 -ar 44100` → `oggenc` (vorbis-tools); see CONTRACT.md §9.2. **Any
